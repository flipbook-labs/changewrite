...
- Run `changewrite lint` to validate every pending entry locally. The `check` command runs the same validation before enforcing that a pull request adds an entry.
